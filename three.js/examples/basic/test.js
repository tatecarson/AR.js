...
function init() {
  ...
  t.position.set(-10, 80, 20),
    t.target.position.set(-200, 88, 10),
    (t.castShadow = !0),
    (t.shadowDarkness = 0.2),
    (t.shadowBias = 1e-5),
    (helper = new THREE.SpotLightHelper(t)),
    (t.shadowMapWidth = 2048),
    (t.shadowMapHeight = 2048);
  var n = new THREE.SpotLight(0, 0.95);
  n.position.set(40, 80, 30),
    n.target.position.set(0, -20, -30),
    (n.castShadow = !0),
    (n.shadowDarkness = 0.2),
    (t.shadowBias = 0.001);
  ...
  (a.castShadow = !0),
    (a.shadowDarkness = 0.2),
    (a.shadowMapWidth = 2048),
    (a.shadowMapHeight = 2048);
  new THREE.DirectionalLightHelper(a, 1);
  var o = new THREE.PointLight(10824509, 0.7, 100);
  o.position.set(110, -5, 10),
    (o.shadowDarkness = 0.4),
    (o.shadowColor = 8018261);
  ...
  (d.rotation.x = 8 * -Math.PI / 24),
    (d.rotation.z = 5 * Math.PI / 16),
    (d.position.x = -20),
    (d.position.y = 10),
    (d.receiveShadow = !0);
  var l = new THREE.Mesh(c, s);
  (l.rotation.x = 8 * -Math.PI / 24),
    (l.rotation.z = 5 * Math.PI / 16),
    (l.position.x = 30),
    (l.position.y = -8),
    (l.receiveShadow = !0);
  var h = new THREE.Mesh(c, s);
  (h.rotation.x = 8 * -Math.PI / 24),
    (h.rotation.z = 5 * Math.PI / 16),
    (h.position.x = 80),
    (h.position.y = -26),
    (h.receiveShadow = !0);
  var y = new THREE.Mesh(c, s);
  (y.rotation.x = 8 * -Math.PI / 24),
    (y.rotation.z = 5 * Math.PI / 16),
    (y.position.x = 130),
    (y.position.y = -43),
    (y.receiveShadow = !0);
  ...
  m.position.set(-20, 38, 0),
    (m.rotation.x = Math.PI / 9),
    (m.rotation.y = -Math.PI / 7),
    m.scale.set(1.5, 1.5, 1.5);
  var M = new THREE.Line(p, w);
  M.position.set(31, 20, 0),
    (M.rotation.x = Math.PI / 9),
    (M.rotation.y = -Math.PI / 6),
    M.scale.set(1.5, 1.64, 1.5);
  var E = new THREE.Line(p, w);
  E.position.set(81, 2, 0),
    (E.rotation.x = Math.PI / 9),
    (E.rotation.y = -Math.PI / 5),
    E.scale.set(1.6, 1.7, 1.5);
  ...
  ((intersectionPlane = new THREE.Mesh(v, H)).visible = !1),
  ...
  window.addEventListener("resize", onWindowResize, !1),
    console.log(
      "Check Out My Works Here:" + "%c www.yanlinma.com ",
      "background: #355493; color: #fff"
    ),
...
function generateSound() {
  ...
  for (
    var o = cylinder1.position.clone(), i = 0;
    i < cylinder1.geometry.vertices.length;
    i++
  ) {
    var r = cylinder1.geometry.vertices[i]
        .clone()
        .applyMatrix4(cylinder1.matrix)
        .sub(cylinder1.position),
      s = new THREE.Raycaster(o, r.clone().normalize()).intersectObjects(
        collidableMeshList
      );
    if (s.length > 0 && s[0].distance < r.length()) {
      switch (((e.innerHTML = "contact1:  " + s[0].distance),
      (plane2.material = new THREE.MeshLambertMaterial({ color: 13837885 })),
  ...
  for (
    var c = cylinder2.position.clone(), d = 0;
    d < cylinder2.geometry.vertices.length;
    d++
  ) {
    var l = cylinder2.geometry.vertices[d]
        .clone()
        .applyMatrix4(cylinder2.matrix)
        .sub(cylinder2.position),
      h = new THREE.Raycaster(c, l.clone().normalize()).intersectObjects(
        collidableMeshList
      );
    if (h.length > 0 && h[0].distance < l.length()) {
      switch (((t.innerHTML = "contact2:  " + h[0].distance),
      Math.floor(h[0].distance))) {
        case 5:
        case 6:
        case 7:
          note1 = "C6";
  ...
  for (
    var y = cylinder3.position.clone(), w = 0;
    w < cylinder2.geometry.vertices.length;
    w++
  ) {
    var p = cylinder3.geometry.vertices[w]
        .clone()
        .applyMatrix4(cylinder3.matrix)
        .sub(cylinder3.position),
      m = new THREE.Raycaster(y, p.clone().normalize()).intersectObjects(
        collidableMeshList
      );
    if (m.length > 0 && m[0].distance < p.length()) {
      switch (((n.innerHTML = "contact3:  " + m[0].distance),
      Math.floor(m[0].distance))) {
        case 5:
        case 6:
        case 7:
          note3 = "C3";
  ...
  for (
    var M = cylinder4.position.clone(), E = 0;
    E < cylinder4.geometry.vertices.length;
    E++
  ) {
    var g = cylinder4.geometry.vertices[E].clone()
        .applyMatrix4(cylinder4.matrix)
        .sub(cylinder4.position),
      v = new THREE.Raycaster(M, g.clone().normalize()).intersectObjects(
        collidableMeshList
      );
    if (v.length > 0 && v[0].distance > 4 && v[0].distance < g.length()) {
      switch (((a.innerHTML = "contact4:  " + v[0].distance),
      Math.floor(v[0].distance))) {
        case 5:
          note4 = "C3";
          break;
        case 6:
...
function changeGeoColor() {
  for (var e = 0; e < mesh.length; e++)
    (Math.abs(mesh[e].position.y - cylinder1.position.y) < 80 &&
      Math.abs(mesh[e].position.x - cylinder1.position.x) < 30) ||
    (Math.abs(mesh[e].position.y - cylinder2.position.y) < 80 &&
      Math.abs(mesh[e].position.x - cylinder2.position.x) < 30) ||
    (Math.abs(mesh[e].position.y - cylinder3.position.y) < 80 &&
      Math.abs(mesh[e].position.x - cylinder3.position.x) < 30) ||
    (Math.abs(mesh[e].position.y - cylinder4.position.y) < 180 &&
      Math.abs(mesh[e].position.x - cylinder4.position.x) < 30)
...
(plane2.rotation.x = 8 * -Math.PI / 24),
  (plane2.rotation.z = 5 * Math.PI / 16),
  (plane2.position.x = 30),
  (plane2.position.y = -8),
  (plane2.receiveShadow = !0);
